...
project=$sdk/Projects/PY32F002A-STK/Example/USART/USART_HyperTerminal_Polling
build=$root/build/official-usart
mkdir -p "$build/obj"
...
$sdk/Drivers/PY32F0xx_HAL_Driver/Src/py32f0xx_hal_usart.c
...
arm-none-eabi-gcc -mcpu=cortex-m0plus -mthumb -nostdlib \
    ...
    "$build/obj/startup.o" $objects -lgcc -o "$build/usart_polling.elf"
arm-none-eabi-objcopy -O binary "$build/usart_polling.elf" \
    "$build/usart_polling.bin"
arm-none-eabi-size "$build/usart_polling.elf"
