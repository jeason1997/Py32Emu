...
project=$sdk/Projects/PY32F002A-STK/Example/EXTI/EXTI_IT
build=$root/build/official-exti
mkdir -p "$build/obj"
...
arm-none-eabi-gcc -mcpu=cortex-m0plus -mthumb -nostdlib \
    ...
    "$build/obj/startup.o" $objects -lgcc -o "$build/exti_it.elf"
arm-none-eabi-size "$build/exti_it.elf"
