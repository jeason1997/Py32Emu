...
project=$sdk/Projects/PY32F002A-STK/Example/ADC/ADC_ContinuousConversion_TriggerSW_Vrefint
build=$root/build/official-adc
mkdir -p "$build/obj"
...
$sdk/Drivers/PY32F0xx_HAL_Driver/Src/py32f0xx_hal_adc.c
"
...
arm-none-eabi-gcc -mcpu=cortex-m0plus -mthumb -nostdlib \
    ...
    "$build/obj/startup.o" $objects -lgcc -o "$build/adc_vrefint.elf"
arm-none-eabi-objcopy -O binary "$build/adc_vrefint.elf" "$build/adc_vrefint.bin"
arm-none-eabi-size "$build/adc_vrefint.elf"
