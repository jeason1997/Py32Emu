...
project=$sdk/Projects/PY32F002A-STK/Example/GPIO/GPIO_Toggle
build=$root/build/official-gpio
mkdir -p "$build/obj"
...
startup=$root/tests/firmware/startup_py32f002a.s
arm-none-eabi-gcc -mcpu=cortex-m0plus -mthumb -c "$startup" \
    -o "$build/obj/startup.o"
arm-none-eabi-gcc -mcpu=cortex-m0plus -mthumb -nostdlib \
    -Wl,--gc-sections -T "$root/tests/firmware/official_py32f002a.ld" \
    "$build/obj/startup.o" $objects -lgcc -o "$build/gpio_toggle.elf"
arm-none-eabi-objcopy -O binary "$build/gpio_toggle.elf" \
    "$build/gpio_toggle.bin"
arm-none-eabi-size "$build/gpio_toggle.elf"
