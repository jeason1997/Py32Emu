...
## 2026-08-14：目标口径纠正与 Agent 规则分层

- 纠正此前“100%”的歧义：它只表示首个功能级版本完成，不表示官方 SDK 全部示例通过。
- 按“逐项运行 PY32F002A-STK 官方示例”的严格口径，当前整体进度暂估约 70%；后续建立完整矩阵后改用可计算百分比。
- 参考 PicEmu，为根目录、`src/`、`examples/`、`frontends/`、`frontends/web/`、`tests/` 和 `docs/` 增加分层 `AGENTS.md`。
- 新规则明确：编译成功、到达步数上限和代表性例程通过均不能替代官方示例全集证据。

当前阶段：Agent 规则补充 100%。下一阶段建立官方示例目录快照和兼容矩阵，逐项计算真实完成率。
