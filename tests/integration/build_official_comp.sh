...
case "$variant" in
IT) name=COMP_CompareGpioVsVrefint_IT ;;
Polling) name=COMP_CompareGpioVsVrefint_Polling ;;
*) echo "unknown COMP variant: $variant" >&2; exit 2 ;;
esac
project=$sdk/Projects/PY32F002A-STK/Example/COMP/$name
build=$root/build/official-comp-$variant
mkdir -p "$build/obj"
...
$sdk/Drivers/PY32F0xx_HAL_Driver/Src/py32f0xx_hal_comp.c
"
objects=""
...
arm-none-eabi-gcc -mcpu=cortex-m0plus -mthumb -nostdlib \
    ...
    "$build/obj/startup.o" $objects -lgcc -o "$build/comp.elf"
arm-none-eabi-size "$build/comp.elf"
