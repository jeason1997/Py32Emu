...
project=$sdk/Projects/PY32F002A-STK/Example/SPI/SPI_TwoBoards_FullDuplexMaster_IT
build=$root/build/official-spi
mkdir -p "$build/obj"
...
$sdk/Drivers/PY32F0xx_HAL_Driver/Src/py32f0xx_hal_spi.c
"
...
arm-none-eabi-gcc -mcpu=cortex-m0plus -mthumb -nostdlib \
    ...
    "$build/obj/startup.o" $objects -lgcc -o "$build/spi_master.elf"
arm-none-eabi-objcopy -O binary "$build/spi_master.elf" \
    "$build/spi_master.bin"
arm-none-eabi-size "$build/spi_master.elf"
