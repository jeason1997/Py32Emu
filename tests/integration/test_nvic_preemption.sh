...
build=$root/build/nvic-preemption
mkdir -p "$build"
arm-none-eabi-gcc -mcpu=cortex-m0plus -mthumb -Os -ffreestanding \
    ...
    "$root/tests/firmware/nvic_preemption.c" \
    "$root/tests/firmware/runtime.c" -lgcc -o "$build/nvic_preemption.elf"
output=$($root/build/py32emu "$build/nvic_preemption.elf" --steps 10000)
printf '%s\n' "$output"
