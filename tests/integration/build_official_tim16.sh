...
project=$sdk/Projects/PY32F002A-STK/Example/TIM16/TIM16_ARR
build=$root/build/official-tim16
mkdir -p "$build/obj"
...
arm-none-eabi-gcc -mcpu=cortex-m0plus -mthumb -nostdlib \
    ...
    "$build/obj/startup.o" $objects -lgcc -o "$build/tim16_arr.elf"
arm-none-eabi-objcopy -O binary "$build/tim16_arr.elf" \
    "$build/tim16_arr.bin"
arm-none-eabi-size "$build/tim16_arr.elf"
